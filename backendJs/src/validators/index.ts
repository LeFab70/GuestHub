...
const isValidCUID = (value: string): boolean => {
  // CUID pattern: starts with 'c', followed by 24 characters (letters, numbers, some special chars)
  const cuidPattern = /^c[a-z0-9]{24}$/;
  return cuidPattern.test(value);
};

const departmentExists = async (value: string): Promise<boolean> => {
  try {
    ...
    logger.error('Error checking department existence:', error);
    return false;
  }
};
    ...
    body('departmentId')
      ...
      .custom((value) => {
        if (value && !isValidCUID(value)) {
          throw new Error('Department ID must be a valid CUID');
        }
        return true;
      })
      .custom(async (value) => {
        if (value) {
          const exists = await departmentExists(value);
          if (!exists) {
