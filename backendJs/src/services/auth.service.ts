...
export class AuthService {
  ...
  generateAccessToken(user: AuthUser): string {
    const payload = {
      ...
      email: user.email,
      role: user.role, // Utiliser role au lieu de roles
  ...
  async createUserWithRole(userData: CreateUserRequest & { role: 'ADMIN' | 'RECEPTIONNISTE' }): Promise<User> {
    try {
      // Check if user already exists
      const existingUser = await prisma.user.findFirst({
        where: {
      ...
      logger.info('User created with role successfully', { 
        userId: user.id, 
        email: user.email, 
        role: user.role 
      });
      ...
      logger.error('User creation with role failed:', error);
      throw error;
    }
  }

  // Create admin user
  async createAdmin(userData: Omit<CreateUserRequest, 'role'>): Promise<User> {
    return this.createUserWithRole({ ...userData, role: 'ADMIN' });
  }

  // Create receptionist user
  async createReceptionist(userData: Omit<CreateUserRequest, 'role'>): Promise<User> {
    return this.createUserWithRole({ ...userData, role: 'RECEPTIONNISTE' });
  }
  ...
  async login(loginData: LoginRequest): Promise<LoginResponse> {
    try {
      ...
      if (user.passwordResetRequired) {
        // Return special response indicating password reset is required
        return {
          user: {
            id: user.id,
            username: user.login,
  ...
  async resetUserPassword(userId: string, adminId: string): Promise<{ tempPassword: string; userEmail: string }> {
    try {
      // Get user info first
      const user = await prisma.user.findUnique({
        where: { id: userId }
      ...
      return { tempPassword, userEmail: user.email };
    } catch (error) {
