...
export class DepartmentController {
  ...
  async createDepartmentWithSuggestion(req: Request, res: Response, next: NextFunction): Promise<Response | void> {
    try {
      const departmentData: CreateDepartmentRequest = req.body;
      
      const department = await departmentService.createDepartmentWithSuggestion(departmentData);
      
      const response: ApiResponse = {
        success: true,
        data: department,
        message: 'Department created successfully with automatic name resolution',
        statusCode: 201
      };

      return res.status(201).json(response);
    } catch (error: any) {
      logger.error('Create department with suggestion failed:', error);
